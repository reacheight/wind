...
const PowerTreads = (props) => {
  const ptInfo = Object.keys(props.powerThreadsAbilityUsage).map((id) => {
    ...
    return <li key={'ptAbility' + id}>
      <span className={styles.heroName}>{props.heroes[id]}</span> used {usageCount[0]} spells total with PT, {usageCount[1]} on <span className={styles.intelligence}>Intelligence</span>
    </li>
  })

  const notOnStrengthInfo = props.ptNotOnStrength.map(entry =>
    <li key={'ptNotOnStrength' + entry[0]}>
      <span className={styles.heroName}>{props.heroes[entry[1]]}</span> <span className={styles.gray}>died</span> with PT not on <span className={styles.red}>Strength</span> at {entry[0]}
    </li>
  )
