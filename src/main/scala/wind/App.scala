    ...
    replayLocation.flatMap(location => {
      ReplayDownloader.downloadReplay(location, compressedReplayPath).map(_ => {
        ...
        val responseData = ujson.Obj(
          ...
          "deathsWithBKB" -> result.deathsWithBKB.map { case (time, id) => (time.toString, id.toString) },
          "deathsWithEssenceRing" -> result.deathsWithEssenceRing.map { case (time, id) => (time.toString, id.toString) }
        )
